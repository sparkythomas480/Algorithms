...
public class SelectionSortTests
{
    ...
    {
        var exception = Assert.Throws<ArgumentNullException>(() => SelectionSort.Do<double>(null!));
    ...
    {
        var exception = Assert.Throws<ArgumentNullException>(() => SelectionSort.Do(null!, Comparer<double>.Default));
        Assert.Equal("elements", exception.ParamName);
        Assert.Equal("The elements argument must not be null. (Parameter 'elements')", exception.Message);
    }
    ...
    {
        var exception = Assert.Throws<ArgumentNullException>(() => SelectionSort.Do(new List<double>(), null!));
    ...
    {
        var toSort = new List<int>();
        SelectionSort.Do(toSort, Comparer<int>.Default);
        Assert.Empty(toSort);
    }
